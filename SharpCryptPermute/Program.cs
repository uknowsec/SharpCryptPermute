...
namespace SharpCryptPermute
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (mod.Contains("-enc"))
            {
                byte[] webshell = File.ReadAllBytes(filestring);
                int length = webshell.Length;
                CryptPermute(webshell, length, false);
                string str = Convert.ToBase64String(webshell);
                Console.WriteLine("[+] Encrypt String: \n{0}", str);
            }
            else if (mod.Contains("-dec"))
            {
                byte[] webshell = Convert.FromBase64String(filestring);
                int length = webshell.Length;
                CryptPermute(webshell, length, true);
                string str = Encoding.Default.GetString(webshell);
